...
- **Interior Gateway Protocols (IGPs)** : These protocols are used to exchange routing information within a single autonomous system (AS) or network. Example includes :
...
Interior Gateway Protocols are routing protocols that operate **within a single autonomous system (AS)**. IGPs are used to exchange routing information and make routing decisions within the boundaries of a single AS. There are two types of routing algorithms in IGPs :
...
Distance vector routing protocol as an analogy, similar to a sign board. If we want to go to Kuala Lumpur from Pahang and we don't have a map, we must rely on the sign board. If the sign board tells us that Kuala Lumpur is in the West direction and 100 km away (<- Kuala Lumpur 100 km), that distance and direction are **not always true**. Distance vector protocols doesn't have a full view of the path to get to a destination network. In other words, it doesn't have a "*road map*" of the network or area; it is only aware of what its neighbor has told it (routing by rumour).
