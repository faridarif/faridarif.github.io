...
- **Ethernet Ports (LAN Ports)** : These ports are used to connect devices within the same local network (LAN) using Ethernet cables. They provide high-speed data transfer between devices and are labelled as FastEthernet (10/100 Mbps) or GigabitEthernet (1 Gbps) ports.
...
In Cisco IOS terminology, the default route is often referred to as the "*gateway of last resort*". It's the route that a router uses when it doesn't know how to reach a specific destination. A default route ensures that data packets are **not dropped** when a router doesn't have a specific route for a destination.
