...
HTTP adalah singkatan bagi *Hypertext Transfer Protocol*.Tujuan penggunaan HTTP adalah bagi memindahkan data diantara *client* (pelayar/browser) dan *server*.Terdapat 2 jenis pemindahan data dalam HTTP iaitu,permintaan (request) dan respons (response)
...
Yang terakhir adalah *body*.Tidak semua request mempunyai body.*HTTP method* GET,HEAD, atau OPTIONS, tidak memerlukan *body*.Jika kita menggunakan *HTTP method* POST,bermaksud kita hendak menghantar sesuatu kepada *server*,maka kita perlu memerlukan *body* untuk HTTP method POST
...
Start-line pada *HTTP Response*,ia mengandungi HTTP version dan HTTP status code (seperti 200 OK, 304 Not Modified, 404 Not Found dan sebagainya)
...
Sama seperti *HTTP request*,headers bagi HTTP response mengandungi maklumat tentang kandungan respons itu dan mempunya request headers,general header dan representation headers

### Body

*Body* bagi HTTP response adalah kandungan data yang kita minta (request) seperti gambar,dokumen,fail,HTML dan sebagainya
